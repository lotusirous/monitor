...
//                logger.debug("lineCnt:[{}]", lineCnt);
                ...
                channelExec = (ChannelExec) session.openChannel("exec");
//                channelExec.setCommand("sed -n \'" + startPoint + "," + endPoint + "p" +"\' " + "/home/ec2-user/gruvoice/collector.log");
                logger.debug("INSTANCE : [{}], CHECKING LINE BETWEEN :[{}] ~ [{}]", instance.getName(), startPoint, endPoint);
                channelExec.setCommand("sed -n \'" + startPoint + "," + endPoint + "p" +"\' " + instance.getLogPath());
                ...
                for (String notiKeyword : notiKeywords) {
                    if (response.toString().contains(notiKeyword)) {
                        logger.debug("[{}] AN {} HAS OCCURRED PLEASE CHECK EMAIL", instance.getName(), notiKeyword);
//                        mail = new Mail(mail.getFrom(), mail.getTo(), mail.getTo(), mail.getHost(), mail.getPort(), mail.getAccount(), mail.getPassword());
    ...
    public void connect() {
        ...
        try {
            ...
            /* Auth */
//            if (instance.getAuthKeyPath() != null && !(instance.getAuthKeyPath().isEmpty())){
...
//            }
            ...
            logger.error("FAIL TO CONNECT TO INSTANCE([{}]), HOST:[{}] ", instance.getName(), instance.getHost());
        }
    }
